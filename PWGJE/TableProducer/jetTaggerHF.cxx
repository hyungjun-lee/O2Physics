...
struct JetTaggerHFTask {
  ...
  std::vector<std::vector<float>> vecParamsIncJetMC_CCDB;
  ...
  void init(InitContext const&)
  {
    ...
    if (usepTcategorize) {
      ...
      for (int i = 0; i < 7; i++) {
        CCDB_ResoFunc.push_back(ccdbApi.retrieveFromTFileAny<TF1>(IPparameterPathsCCDB->at(i), metadata, -1));
      }
    }
    ...
    switch (resoFuncMatch) {
      ...
      case 6: // TODO
        ...
        for (size_t j = 0; j < CCDB_ResoFunc.size(); j++) {
          std::vector<float> params;
          if (CCDB_ResoFunc[j]) {
            for (int i = 0; i < 9; i++) {
              params.emplace_back(CCDB_ResoFunc[j]->GetParameter(i));
            }
          }
          vecParamsIncJetMC_CCDB.emplace_back(params);
        }
    ...
    for (const auto& params : vecParamsIncJetMC_CCDB) {
      fSignImpXYSigIncJetMC_CCDB_vec.emplace_back(jettaggingutilities::setResolutionFunction(params));
    }
